  ...
  <v-dialog v-model="isOpen" width="500">
    ...
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click.stop="addsubExpertist">บันทึก</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    async addsubExpertist() {
      if (!this.name) return
      try {
        await this.$axios.$post('/select/expertist/sub', {
          _id: this.item._id,
          subName: this.name,
        })
      } catch (error) {
        this.$toast.error(`ชื่อความเชี่ยวชาญ (ย่อย) : ${this.name} มีอยู่แล้ว`)
      } finally {
        this.name = ''
        this.$emit('refetch')
        this.isOpen = false
